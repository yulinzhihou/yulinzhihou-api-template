...
```
- 第二步：复制 `.env.sample` 为 `.env` 并创建一个空的数据库。配置好 `mysql` , `redis` 账号密码端口等配置参数，

- 第三步：安装依赖会初始化数据表以及基础数据，`admin`,`menu`,`role` `api_log` 表里面等相关数据，
```shell
...
- 第四步：增加路由访问，或者直接封装成 `php think command` 进行调用
```php
# 封装 command 可以参考本脚手架里面的 yc yv ym 等命令的封装。
# 这里调用方式为增加路由访问
```